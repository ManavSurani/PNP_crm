    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-900 p-8 rounded-xl border border-slate-200 dark:border-white/8 shadow-sm relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h1 className="text-2xl font-semibold text-slate-900 dark:text-white tracking-tight">Customer Directory</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Converted leads and active clients who have purchased your services.</p>
        </div>
        <div className="relative z-10 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-100 dark:border-emerald-500/30 px-4 py-2 rounded-lg">
           <span className="text-emerald-700 dark:text-emerald-300 font-bold text-sm">{filteredCustomers.length} Total Customers</span>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center gap-4">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          <button 
            ...
            className={cn(
              "flex items-center gap-2 px-4 py-2.5 border rounded-lg text-sm font-medium transition shadow-sm cursor-pointer",
              showFilters 
          ...
          {(searchTerm !== "" || filters.source !== "ALL" || filters.service !== "ALL" || filters.startDate !== "" || filters.endDate !== "" || sortBy !== "NEWEST") && (
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/8 rounded-lg text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition shadow-sm cursor-pointer"
            >
      ...
      {showFilters && (
        <div className="bg-emerald-50/50 dark:bg-slate-900 p-3 rounded-xl border border-emerald-100 dark:border-white/8 shadow-sm animate-in slide-in-from-top-2 duration-200 shrink-0">
          <div className="flex flex-wrap items-end gap-3">
            <div className="min-w-[140px] flex-1">
              ...
              <select 
                className="w-full rounded-lg border border-emerald-200 dark:border-slate-700 bg-white/70 dark:bg-[#161f32] py-1.5 px-3 text-xs text-slate-800 dark:text-slate-200 focus:bg-white dark:focus:bg-[#1a233a] focus:border-emerald-500 outline-none transition-all cursor-pointer"
            ...
            <div className="flex-[1.5] flex items-center gap-2">
              <div className="flex-1">
                <label className="block text-[9px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-tight mb-1 ml-1">Conversion From</label>
                <input 
                  type="date"
                  className="w-full rounded-lg border border-emerald-200 dark:border-slate-700 bg-white/70 dark:bg-[#161f32] py-1.5 px-3 text-xs text-slate-800 dark:text-slate-200 focus:bg-white dark:focus:bg-[#1a233a] focus:border-emerald-500 outline-none transition-all"
                  value={filters.startDate}
                  onChange={e => setFilters({...filters, startDate: e.target.value})}
                />
              </div>
              <div className="flex-1">
                <label className="block text-[9px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-tight mb-1 ml-1">Conversion To</label>
                <input 
                  type="date"
                  className="w-full rounded-lg border border-emerald-200 dark:border-slate-700 bg-white/70 dark:bg-[#161f32] py-1.5 px-3 text-xs text-slate-800 dark:text-slate-200 focus:bg-white dark:focus:bg-[#1a233a] focus:border-emerald-500 outline-none transition-all"
          ...
          <div className="overflow-auto flex-1 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-700" style={{ maxHeight: 'calc(100vh - 320px)' }}>
            <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800 table-fixed" style={{ minWidth: '800px' }}>
              <thead className="bg-slate-50/90 dark:bg-[#161f32]/95 border-b border-slate-100 dark:border-slate-800 sticky top-0 z-20 backdrop-blur-sm">
                <tr>
                  <th scope="col" className="w-[30%] py-4 pl-8 pr-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Client Profile</th>
                  <th scope="col" className="w-[25%] px-3 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Purchased Service</th>
                  <th scope="col" className="w-[20%] px-3 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Conversion Date</th>
                  <th scope="col" className="w-[15%] px-3 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Handler</th>
                ...
                {filteredCustomers.length === 0 ? (
                  ...
                  filteredCustomers.map((customer) => {
                    ...
                    return (
                      ...
                      >
                        <td onClick={() => router.push(`/customers/${customer.id}`)} className="whitespace-nowrap py-5 pl-8 pr-3">
                          <div className="flex items-center">
                            <div className="h-10 w-10 flex-shrink-0 bg-emerald-50 dark:bg-emerald-950/40 rounded-lg flex items-center justify-center text-emerald-600 dark:text-emerald-400 font-semibold border border-emerald-100 dark:border-emerald-500/30">
                              {displayName.charAt(0)}
                            </div>
                            <div className="ml-4">
                              <div className="text-sm font-semibold text-slate-900 dark:text-white group-hover:text-emerald-500 transition-colors flex items-center gap-2">
                                {displayName}
                                {customer.project?.name && (
                                  <span className="text-[9px] font-black bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-400 px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-700 uppercase tracking-tighter">Project</span>
                                )}
                              </div>
                      ...
                      <td onClick={() => router.push(`/customers/${customer.id}`)} className="whitespace-nowrap px-3 py-5">
                        <div className="mt-1.5 text-xs font-medium text-slate-600 dark:text-slate-300">{format(new Date(customer.project?.startedOn || customer.createdAt), "dd MMM yyyy")}</div>
                      </td>
                      <td onClick={() => router.push(`/customers/${customer.id}`)} className="whitespace-nowrap px-3 py-5">
                        <div className="text-xs font-semibold text-slate-900 dark:text-slate-200 flex items-center gap-2">
                          <div className="h-5 w-5 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center text-[10px] border border-white dark:border-slate-700">
                      ...
                      <td className="relative whitespace-nowrap py-5 pl-3 pr-8 text-right">
                         <button 
                           ...
                           className="text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/40 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 border border-emerald-100 dark:border-emerald-500/30 cursor-pointer"
                         >
