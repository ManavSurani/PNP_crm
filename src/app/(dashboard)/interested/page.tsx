...
import {
  Star, Loader2, Search, Archive, RotateCcw, ArrowRight
} from "lucide-react";
    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-10">
      ...
      <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-2">
          ...
          <div className="inline-flex items-center p-1 bg-slate-100 rounded-xl border border-slate-200/80 shadow-inner">
            <button
            ...
            <h1 className="text-2xl font-semibold text-slate-900 tracking-tight flex items-center gap-3">
              {activeTab === "INTERESTED" ? (
                ...
                <>
                  ...
                  Archived Leads <span className="text-xs font-semibold text-indigo-600 bg-indigo-50 border border-indigo-200 px-2 py-0.5 rounded-full uppercase tracking-wider">Passively Active</span>
                </>
              )}
            </h1>
        ...
        <div className="relative z-10 shrink-0">
          {activeTab === "INTERESTED" ? (
            ...
            <div className="bg-slate-900 text-white px-4 py-2 rounded-lg shadow-sm border border-slate-800">
              ...
              <div className="p-6 space-y-4">
                <div className="flex items-center justify-between">
                   <div className="relative h-10 w-10 shrink-0">
                     <div className="h-10 w-10 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-center font-bold text-slate-600">
                   ...
                   {activeTab === "ARCHIVED" && (
                     <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-semibold bg-slate-100 text-slate-700 border border-slate-200">
                       <Archive className="h-3 w-3 text-slate-500" /> Paused
                     </span>
                   )}
                </div>
                <div>
                   ...
                   <p className="text-xs text-slate-400 font-semibold uppercase tracking-widest mt-1">{lead.serviceType?.replace(/_/g, " ") || "Other"}</p>
                </div>
                   ...
                   <div>
                    ...
                    <div>
                      <p className="text-[9px] font-bold text-indigo-400 uppercase tracking-widest">
                        {activeTab === "INTERESTED" ? "Next Follow-up" : "Current Status"}
                      </p>
                ...
                <div className="flex gap-2 pt-2">
                   {activeTab === "ARCHIVED" && (
                     <button
                       ...
                       className="flex-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 py-2.5 rounded-lg text-[11px] font-bold uppercase tracking-wider text-center transition-all flex items-center justify-center gap-1.5 active:scale-95 disabled:opacity-50"
                   ...
                   <Link href={`/leads/${lead.id}`}
                     className={cn(
                       "bg-white hover:bg-slate-50 text-slate-900 border border-slate-200 py-2.5 rounded-lg text-[11px] font-bold uppercase tracking-wider text-center transition-all flex items-center justify-center gap-1.5",
