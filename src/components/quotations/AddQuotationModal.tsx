...
import { X, Plus, Check, Loader2, Phone, Briefcase } from "lucide-react";
...
export default function AddQuotationModal({ isOpen, onClose, customerId, onSuccess }: AddQuotationModalProps) {
  ...
  const handleAddVendor = async () => {
    if (!newVendorName.trim() || !newVendorPhone.trim() || !selectedField) return;
    setIsLoading(true);
    try {
            ...
            <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
              ...
              {!isAddingVendor ? (
                <div className="space-y-3">
                  <div className="max-h-[200px] overflow-y-auto space-y-2 pr-1">
                    {vendors.length === 0 && !isLoading && (
                      <p className="text-center py-8 text-xs text-slate-400 italic">No vendors found for this field.</p>
                    )}
                    {vendors.map(v => (
                      <button
                        key={v.id}
                ...
                <div className="space-y-4 p-4 bg-emerald-50/50 rounded-xl border border-emerald-100">
                  <div className="space-y-3">
                    <div>
                      <label className="text-[10px] font-black text-emerald-700 uppercase tracking-wider ml-1 mb-1 block">Vendor Name</label>
                      <input 
                        autoFocus
                        placeholder="e.g. John's Electricals"
                        className="w-full rounded-lg border border-emerald-200 p-2.5 text-sm focus:border-emerald-500 outline-none"
                        value={newVendorName}
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="flex-1 bg-emerald-600 text-white rounded-lg py-2 text-xs font-bold hover:bg-emerald-700 disabled:opacity-50"
                    >
                      {isLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin mx-auto" /> : "SAVE & SELECT"}
                    </button>
                    <button 
